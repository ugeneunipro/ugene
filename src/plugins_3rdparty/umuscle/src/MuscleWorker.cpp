...
void MuscleWorkerFactory::init() {
    ...
    Descriptor mod(MODE_ATTR, MuscleWorker::tr("Mode"), MuscleWorker::tr("Selector of preset configurations, that give you the choice of optimizing accuracy,"
                                                                         " speed, or some compromise between the two. The default favors accuracy."));
    Descriptor sd(STABLE_ATTR, MuscleWorker::tr("Stable order"), MuscleWorker::tr("Do not rearrange aligned sequences (-stable switch of MUSCLE). "
    ...
    Descriptor mi(MAX_ITERATIONS_ATTR, MuscleWorker::tr("Max iterations"), MuscleWorker::tr("Maximum number of iterations."));
    Descriptor ra(RANGE_ATTR, MuscleWorker::tr("Region to align"), MuscleWorker::tr("Whole alignment or column range e.g. <b>1..100</b>."));
    ...
    proto->setIconParameters(IconParameters("umuscle", "muscle_16.png", false));
...
QString MusclePrompter::composeRichDoc() {
    ...
    switch (getParameter(MODE_ATTR).toInt()) {
        case 0:
            preset = DefaultModePreset().name;
            break;
        case 1:
            preset = LargeModePreset().name;
            break;
        case 2:
            preset = RefineModePreset().name;
            break;
    }
...
Task* MuscleWorker::tick() {
    if (input->hasMessage()) {
        ...
        switch (mode) {
            case 0:
                DefaultModePreset().apply(cfg);
                break;
            case 1:
                LargeModePreset().apply(cfg);
                break;
            case 2:
                RefineModePreset().apply(cfg);
                break;
        }
