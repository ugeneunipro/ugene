...
void GenomeAlignerWorkerFactory::init() {
    ...
    QList<Attribute*> attrs;
    {
        ...
        Descriptor indexDir(INDEX_DIR,
                            GenomeAlignerWorker::tr("Genome Aligner index folder"),
                            GenomeAlignerWorker::tr("The folder with the index for the reference sequence."));
        ...
        auto mismatchesAttr = new Attribute(mismatches, BaseTypes::NUM_TYPE(), false, 0);
        mismatchesAttr->addRelation(new VisibilityRelation(ABS_OR_PERC_MISMATCHES_ATTR, QVariant(true)));
        attrs << mismatchesAttr;
        auto ptMismatchesAttr = new Attribute(ptMismatches, BaseTypes::NUM_TYPE(), false, 0);
    ...
    Descriptor desc(ACTOR_ID, GenomeAlignerWorker::tr("Map Reads with UGENE Genome Aligner"), GenomeAlignerWorker::tr("Genome Aligner is a program for mapping short DNA sequence reads"
