...
public:
    ...
    static void clickAnnotationDet(
     ...
     /**
     * Clicks on the center (if @clickShift is 0) of the annotation region in PanView.
     * Locates the region to click by the annotationName and annotationRegionStartPos(visual, starts with 1) that must be one of the location.region.startPos.
     ...
     * @clickShift by default, the center of annotation is being clicked on. This parameter sets the shift from the center (>0 - to the right, <0 - to the left)
     */
    static void clickAnnotationPan(const QString& name, int startPos, int number = 0, bool isDoubleClick = false, Qt::MouseButton button = Qt::LeftButton, int clickShift = 0);
