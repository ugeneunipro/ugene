...
void AnnotatorViewContext::initViewContext(GObjectViewController* v) {
    ...
    auto findRegionsAction = new ADVGlobalAction(av, ":annotator/images/regions.png", tr("Find annotated regions..."), 30);
    connect(findRegionsAction, SIGNAL(triggered()), SLOT(sl_showCollocationDialog()));

    if (customFeaturesAvailable) {
        auto annotatePlasmidAction = new ADVGlobalAction(av, ":annotator/images/plasmid_features.png", tr("Annotate plasmid..."), 31);
