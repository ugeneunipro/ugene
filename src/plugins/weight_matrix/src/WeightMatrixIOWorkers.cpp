...
const Descriptor PWMatrixWorkerFactory::WEIGHT_MATRIX_CATEGORY() {
    return Descriptor("hweightmatrix", WeightMatrixIO::tr("Weight matrix"), "");
}
...
ReadPWMatrixProto::ReadPWMatrixProto(const Descriptor& _desc, const QList<PortDescriptor*>& _ports, const QList<Attribute*>& _attrs)
    ...
    setIconParameters(IconParameters("weight_matrix", "weight_matrix.png"));
}
...
const Descriptor PFMatrixWorkerFactory::FREQUENCY_MATRIX_CATEGORY() {
    return Descriptor("hweightmatrix", WeightMatrixIO::tr("Weight matrix"), "");
}
