...
void DNASequenceGenerator::generateSequence(const QMap<char, qreal> &charFreqs, int length, QByteArray &result) {
    ...
    QVector<qint64> reserveByChar(256, 0);    // Stores how many characters can be generated by char code (uchar(c)).
    ...
    for (char ch : qAsConst(chars)) {
        ...
        reserveByChar[(uchar)ch] = expectedCharCount;
        availableChars.append(ch);
        total += expectedCharCount;
    }
    SAFE_POINT(total >= 0 && total <= length, "Invalid total checksum", );
    reserveByChar[(uchar)chars[0]] += length - total;    // Fill the remaining (rounding error) with the first char.
    ...
    for (int idx = 0; idx < length; idx++) {
        ...
        result[idx] = ch;
        reserveByChar[(uchar)ch]--;
        if (reserveByChar[(uchar)ch] == 0) {
...
DNASequenceGeneratorTask::DNASequenceGeneratorTask(const DNASequenceGeneratorConfig &cfg)
    ...
    if (cfg.useReference()) {
        ...
        generateTask->setSubtaskProgressWeight(0.5);
...
void GenerateDNASequenceTask::run() {
    ...
    for (int seqCount = 0; seqCount < count; seqCount++) {
        ...
        for (int chunkCount = 0; chunkCount < length / window && !isCanceled();
            ...
            CHECK_OP_BREAK(stateInfo);
            const int currentProgress = int(100 * (seqCount + chunkCount * double(window) / double(length)) / double(count));
