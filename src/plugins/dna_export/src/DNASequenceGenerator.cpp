...
void DNASequenceGenerator::generateSequence(const QMap<char, qreal> &charFreqs, int length, QByteArray &result, U2OpStatus &os) {
    ...
    QVector<qint64> quotaByChar(256, 0);  // Keeps counters how many characters can be generated by char code (uchar(c)).
    QList<char> availableChars;  // List of chars that can be generated.
    int total = 0;
    QList<char> chars = charFreqs.keys();
    for (char ch : qAsConst(chars)) {
        qreal percent = charFreqs[ch];
    ...
    for (int idx = 0; idx < length; idx++) {
        SAFE_POINT(!availableChars.isEmpty(), "No available chars left", );
        int avalableCharIndex = qrand() % availableChars.size();
        char ch = availableChars[avalableCharIndex];
        result[idx] = ch;
        quotaByChar[(uchar)ch]--;
        if (quotaByChar[(uchar)ch] == 0) {
            availableChars.removeAt(avalableCharIndex);
        }
    }
}
...
QList<Task *> DNASequenceGeneratorTask::onGenerateTaskFinished() {
    ...
    if (cfg.saveDoc) {
        ...
        for (int sequenceNum = 0, totalSeqCount = seqs.size(); sequenceNum < totalSeqCount;
             ++sequenceNum) {
            const QString seqName = (1 < totalSeqCount) ? (baseSeqName + " " + QString::number(sequenceNum + 1)) : baseSeqName;
...
void DNASequenceGeneratorTask::addSequencesToMsaDoc(Document *source) {
    ...
    for (int sequenceNum = 0, totalSeqCount = seqs.size(); sequenceNum < totalSeqCount; sequenceNum++) {
        QString seqName = (1 < totalSeqCount) ? (baseSeqName + " " + QString::number(sequenceNum + 1)) : baseSeqName;
        // TODO: large sequences will cause out of memory error here
        QByteArray seqContent = con.dbi->getSequenceDbi()->getSequenceData(seqs[sequenceNum].id, U2_REGION_MAX, stateInfo);
...
void GenerateDNASequenceTask::run() {
    ...
    for (int seqCount = 0; seqCount < count; seqCount++) {
        ...
        for (int chunkCount = 0; chunkCount < length / window && !isCanceled(); chunkCount++) {
            DNASequenceGenerator::generateSequence(baseContent, window, sequenceChunk, stateInfo);
            CHECK_OP_BREAK(stateInfo);
        ...
        // append last chunk
        DNASequenceGenerator::generateSequence(baseContent, length % window, sequenceChunk, stateInfo);
        CHECK_OP_BREAK(stateInfo);
        seqImporter.addBlock(sequenceChunk.constData(), sequenceChunk.length(), stateInfo);
        CHECK_OP_BREAK(stateInfo);
