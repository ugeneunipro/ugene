...
QString RemoteDBFetcherPrompter::composeRichDoc() {
    ...
    return RemoteDBFetcherWorker::tr("Reads %1 %2 from <u>%3</u> remote database. %4").arg(sourceDescString).arg(getHyperlink(sourceId, sourceLinkStr)).arg(getHyperlink(DBID_ID, dbid)).arg(saveDirStr);
}
...
void RemoteDBFetcherFactory::init() {
    ...
    if (AppContext::isGUIMode()) {
        proto->setIconParameters(IconParameters("U2Designer", "blue_circle.png", false));
    }
...
void FetchSequenceByIdFromAnnotationFactory::init() {
    ...
    {
        Descriptor outDesc(BasePorts::OUT_SEQ_PORT_ID(), FetchSequenceByIdFromAnnotationWorker::tr("Sequence"), FetchSequenceByIdFromAnnotationWorker::tr("The downloaded sequences"));
