...
void DotPlotViewContext::initViewContext(GObjectViewController* v) {
    ...
    auto act = new ADVGlobalAction(av, IconParameters("dotplot", "dotplot.png"), dotPlotBuildString, 40, ADVGlobalActionFlags(ADVGlobalActionFlag_AddToAnalyseMenu));
    act->setObjectName(BUILD_DOT_PLOT_ACTION_NAME);
    connect(act, SIGNAL(triggered()), SLOT(sl_buildDotPlot()));

    auto tb = new ADVGlobalAction(av, IconParameters("dotplot", "dotplot.png"), dotPlotBuildString, 40, ADVGlobalActionFlags(ADVGlobalActionFlag_AddToToolbar));
