...
DnaAssemblySupport::DnaAssemblySupport() {
    ...
    GUIUtils::setIcon(convertAssemblyToSamAction, IconRef("core", "align.png"));
    ...
    GUIUtils::setIcon(dnaAssemblyAction, IconRef("core", "align.png"));
    ...
    GUIUtils::setIcon(buildIndexAction, IconRef("core", "align.png"));
...
void FilterUnpairedReadsTask::run() {
    ...
    for (int i = 0; i < upstream.size(); i++) {
        ...
        compareFiles(upstream[i].url.getURLString(), downstream[i].url.getURLString(), tmpFileUpstream, tmpFileDownstream);
        CHECK_OP(stateInfo, );
    }
}
...
void FilterUnpairedReadsTask::compareFiles(const GUrl& upstream, const GUrl& downstream, const GUrl& upstreamFiltered, const GUrl& downstreamFiltered) {
    ...
    if (comparator.getUnpairedCount() != 0) {
        stateInfo.addWarning(tr("%1 read pairs were mapped, %2 reads without a pair from files \"%3\" and \"%4\" were skipped.")
                                 .arg(comparator.getPairsCount())
                                 .arg(comparator.getUnpairedCount())
                                 .arg(QFileInfo(upstream.getURLString()).fileName())
                                 .arg(QFileInfo(downstream.getURLString()).fileName()));
    }
}
