...
MsaEditorSequenceArea::MsaEditorSequenceArea(MaEditorWgt* _ui, GScrollBar* hb, GScrollBar* vb)
    ...
    delColAction = new U2Action(IconParameters("core", "msaed_remove_columns_with_gaps.png"), tr("Remove columns of gaps..."), this);
    ...
    removeAllGapsAction = new U2Action(IconParameters("core", "msaed_remove_all_gaps.png"), tr("Remove all gaps"), this);
    ...
    toggleSequenceRowOrderAction = new U2Action(IconParameters("core", "collapse.png"), tr("Switch on/off collapsing"), this);
    ...
    refreshSequenceRowOrder = new U2Action(IconParameters("core", "collapse_update.png"), tr("Update collapsed groups"), this);
    ...
    connect(editor->getMaObject(), &MsaObject::si_alignmentChanged, this, &MsaEditorSequenceArea::sl_updateActions);
...
void MsaEditorSequenceArea::sl_delCol() {
    ...
    if (dlg->result() == QDialog::Accepted) {
        ...
        MsaObject* msaObj = getEditor()->getMaObject();
        int gapCount;
        switch (deleteMode) {
...
void MsaEditorSequenceArea::sl_removeAllGaps() {
    MsaObject* msa = getEditor()->getMaObject();
...
void MsaEditorSequenceArea::sl_createSubalignment() {
    MsaObject* msaObject = getEditor()->getMaObject();
    QList<int> maRowIndexes = editor->getSelectionController()->getSelectedMaRowIndexes();
    const Msa& alignment = msaObject->getAlignment();
...
void MsaEditorSequenceArea::reverseComplementModification(ModificationType& type) {
    ...
    for (int i = 0; i < selectedMaRowIndexes.size(); i++) {
        ...
        MsaRow currentRow = ma->getRow(maRowIndex);
...
QString ExportHighlightingTask::generateExportHighlightingReport() const {
    ...
    const MsaRow& row = msa->getRow(refSeq);
