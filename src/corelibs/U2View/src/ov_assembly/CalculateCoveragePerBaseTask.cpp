...
U2CigarOp CalculateCoveragePerBaseOnRegionTask::nextCigarOp(const QVector<U2CigarOp>& cigarVector, int& index, int& insertionsCount) {
    ...
    do {
        SAFE_POINT_EXT(index < cigarVector.length(), setError("Cigar string: out of bounds"), U2CigarOp_Invalid);
...
QList<Task*> CalculateCoveragePerBaseTask::onSubTaskFinished(Task* subTask) {
    ...
    if (subTask == getLengthTask) {
        ...
        SAFE_POINT_EXT(calculateTask != nullptr, setError("An unexpected subtask"), res);
...
void GetAssemblyLengthTask::run() {
    ...
    SAFE_POINT_EXT(lengthAttribute.value > 0, setError(tr("Assembly length must be greater than zero")), );
