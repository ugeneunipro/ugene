...
void AssemblyReadsArea::drawReads(QPainter& p) {
    ...
    cachedReads.data = model->getReadsFromAssembly(cachedReads.visibleBases, cachedReads.visibleRows.startPos, cachedReads.visibleRows.endPos(), status);
...
void AssemblyReadsArea::sl_onExportReadsOnScreen() {
    if (!cachedReads.data.isEmpty()) {
        ...
        QMessageBox::warning(QApplication::activeWindow(), AssemblyReadsArea::tr("Export visible reads as sequence."), AssemblyReadsArea::tr("There are no reads in the current area. Nothing to export!"), QMessageBox::Ok);
    }
}
