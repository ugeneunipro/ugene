...
U2EntityRef ChromatogramUtils::import(U2OpStatus& os, const U2DbiRef& dbiRef, const QString& folder, const Chromatogram& chromatogram) {
    ...
    U2EntityRef entityRef(dbiRef, dbChromatogram.id);
...
U2EntityRef ChromatogramUtils::getChromatogramIdByRelatedSequenceId(U2OpStatus& os, const U2EntityRef& sequenceRef) {
    ...
    return {sequenceRef.dbiRef, chromatogramsIds.first()};
}
...
U2Region ChromatogramUtils::sequenceRegion2TraceRegion(const Chromatogram& chromatogram, const U2Region& sequenceRegion) {
    ...
    const int traceLength = chromatogram->baseCalls[(int)sequenceRegion.endPos() - 1] - traceStartPos + 1;
...
void ChromatogramUtils::insertBase(Chromatogram& chromatogram, int posUngapped, const QVector<U2MsaGap>& gapModel, int posWithGaps) {
    ...
    chromatogram->prob_A.insert(posUngapped, 0);
    chromatogram->prob_C.insert(posUngapped, 0);
    chromatogram->prob_G.insert(posUngapped, 0);
    chromatogram->prob_T.insert(posUngapped, 0);
...
Chromatogram ChromatogramUtils::getGappedChromatogram(const Chromatogram& chromatogram, const QVector<U2MsaGap>& gapModel) {
    ...
    foreach (const U2MsaGap& gap, gapModel) {
        ...
        for (int i = 0; i < gap.length; i++) {
            ...
            gappedChromatogram->prob_A.insert(startBaseCallIndex + i + 1, gap.length, 0);
            gappedChromatogram->prob_C.insert(startBaseCallIndex + i + 1, gap.length, 0);
            gappedChromatogram->prob_G.insert(startBaseCallIndex + i + 1, gap.length, 0);
            gappedChromatogram->prob_T.insert(startBaseCallIndex + i + 1, gap.length, 0);
        }
