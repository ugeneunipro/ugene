...
static QString NAME_ATTR_PATTERN = "\\s" + U2::WorkflowSerialize::Constants::NAME_ATTR + "\\s*" + U2::WorkflowSerialize::Constants::EQUALS_SIGN;
static QString TYPE_ATTR_PATTERN = "\\s" + U2::WorkflowSerialize::Constants::TYPE_ATTR + "\\s*" + U2::WorkflowSerialize::Constants::EQUALS_SIGN;
static QString BLOCK_START_PATTERN = "[\\s\"]\\" + U2::WorkflowSerialize::Constants::BLOCK_START + "[\\s\"]";
static QString BLOCK_END_PATTERN = "[\\s\\" + U2::WorkflowSerialize::Constants::SEMICOLON + "]\\" + U2::WorkflowSerialize::Constants::BLOCK_END + "\\s";
...
SchemeWrapper::SchemeWrapper(const QString& initPathToScheme, U2ErrorType* result)
    ...
    const QString tmpDirPath = U2::AppContext::getAppSettings()->getUserAppsSettings()->getCurrentProcessTemporaryDirPath();
    U2OpStatusImpl stateInfo;
    pathToScheme = GUrlUtils::prepareTmpFileLocation(tmpDirPath, TMP_FILE_NAME_BASE, WorkflowUtils::WD_FILE_EXTENSIONS.first(), stateInfo);
    ...
    CHECK_EXT(schemeFile.open(QIODevice::WriteOnly | QIODevice::Text),
              schemeContent = QString();
              *result = U2_FAILED_TO_CREATE_FILE, );
    *result = U2_OK;
}
...
U2ErrorType SchemeWrapper::setElementAttribute(const QString& elementName,
    ...
    result = getEnclosingElementBoundaries(elementName, &elementDescStartPosition, &elementDescEndPosition);
    CHECK(U2_OK == result, result);
    ...
    foreach (QString value, valuesTuple) {
        result = setElementAttributeInRange(deepestAttributeName, value, elementDescStartPosition, elementDescEndPosition, replaceIfExists);
        CHECK(U2_OK == result, result);
    }
    return U2_OK;
}
...
U2ErrorType SchemeWrapper::getElementAttribute(const QString& elementName,
    ...
    result = getElementAttributeFromRange(deepestAttributeName, elementDescStartPosition, elementDescEndPosition, attributeValue);
    CHECK(U2_OK == result, result);
    return U2_OK;
}
...
U2ErrorType SchemeWrapper::addActorsBinding(const QString& srcElementName,
    ...
    if (SUBSTRING_NOT_FOUND == bindingDescriptionPosition) {
        ...
        QRegularExpressionMatch match = flowsDescriptionEndPattern.match(schemeContent, bindingDescriptionPosition);
        int lastFlowIndex = match.hasMatch() ? match.capturedEnd(0) - 1 : -1;
        bindingDescriptionPosition = qMax(lastFlowIndex, schemeContent.lastIndexOf(Constants::NEW_LINE,
                                                                    bindingDescriptionPosition));
        CHECK(SUBSTRING_NOT_FOUND != bindingDescriptionPosition, U2_INVALID_SCHEME);
    }
    const QString bindingDescription = HRSchemaSerializer::makeArrowPair(
                                           ...
                                           elementNamesAndIds[dstElementName] + Constants::DOT + dstPortName + Constants::DOT + dstSlotName) +
                                       Constants::NEW_LINE;
...
U2ErrorType SchemeWrapper::saveToFile(QString& path) {
    ...
    CHECK_EXT(resultFile.open(QFile::WriteOnly | QFile::Truncate), path = QString(), U2_FAILED_TO_CREATE_FILE);
    restoreComments();
...
U2ErrorType SchemeWrapper::createSas(const QString& elementType, const QString& inputFilePath, const QString& outputFilePath, SchemeWrapper** sas) {
    ...
    try {
        ...
        result = WorkflowElementFacade::getElementSlotIds(elementType, true, QString(), inputSlotNames);
        CHECK_DEL_OBJECT(result, newSas);
        ...
        result = WorkflowElementFacade::getElementSlotIds(elementType, false, QString(), outputSlotNames);
        CHECK_DEL_OBJECT(result, newSas);
        ...
        result = WorkflowElementFacade::getOutputPortIdForSlot(readerType, elementInputSlot, outputReaderPortId);
        CHECK_DEL_OBJECT(result, newSas);
        // get the element's input port name
        QString inputElementPortId;
        result = WorkflowElementFacade::getInputPortIdForSlot(elementType, elementInputSlot, inputElementPortId);
        CHECK_DEL_OBJECT(result, newSas);
        ...
        result = WorkflowElementFacade::getOutputPortIdForSlot(elementType, elementOutputSlot, outputElementPortId);
        CHECK_DEL_OBJECT(result, newSas);
        // get the writer's output port name
        QString inputWriterPortId;
        result = WorkflowElementFacade::getInputPortIdForSlot(writerType, elementOutputSlot, inputWriterPortId);
        CHECK_DEL_OBJECT(result, newSas);
        // add flow between found ports
        ...
        result = newSas->addActorsBinding(readerName, elementInputSlot, elementName, inputElementPortId + Constants::DOT + elementInputSlot);
        CHECK_DEL_OBJECT(result, newSas);
        // bind the element's and the writer's slots
        result = newSas->addActorsBinding(elementName, elementOutputSlot, writerName, inputWriterPortId + Constants::DOT + elementOutputSlot);
        CHECK_DEL_OBJECT(result, newSas);

        // bind unbinded reader's and writer's slots
        // first get reader's output slots
        QStringList readerSlots;
        result = WorkflowElementFacade::getElementSlotIds(readerType, true, outputReaderPortId, readerSlots);
        CHECK_DEL_OBJECT(result, newSas);
        // then get writer's input slots
        QStringList writerSlots;
        result = WorkflowElementFacade::getElementSlotIds(writerType, true, inputWriterPortId, writerSlots);
        CHECK_DEL_OBJECT(result, newSas);
        // finally bind slots with the same name
        foreach (QString readerSlot, readerSlots) {
            if (writerSlots.contains(readerSlot) && Workflow::BaseSlots::URL_SLOT() != readerSlot) {
                result = newSas->addActorsBinding(readerName, readerSlot, writerName, inputWriterPortId + Constants::DOT + readerSlot);
                CHECK_DEL_OBJECT(result, newSas);
            }
        }
...
int reIndexIn(const QRegularExpression& re, const QString& str, int pos = 0) {
    QRegularExpressionMatch match = re.match(str, pos);
    return match.hasMatch() ? match.capturedStart(0) : SUBSTRING_NOT_FOUND;
}

int reEndIn(const QRegularExpression& re, const QString& str, int pos = 0) {
    QRegularExpressionMatch match = re.match(str, pos);
    return match.hasMatch() ? match.capturedEnd(0) - 1 : SUBSTRING_NOT_FOUND;
}
...
U2ErrorType SchemeWrapper::fillElementNamesFromSchemeContent() {
    ...
    searchStartPosition = reIndexIn(blockStartPattern, schemeContent, searchStartPosition);

    while (SUBSTRING_NOT_FOUND != searchStartPosition) {
        int nextBlockStart = reIndexIn(blockStartPattern, schemeContent, searchStartPosition + 2);
        int blockEnd = reEndIn(blockEndPattern, schemeContent, searchStartPosition + 2);

        CHECK_EXT(SUBSTRING_NOT_FOUND != blockEnd, elementNamesAndIds.clear(), U2_INVALID_SCHEME);

        int elementNameStart = reEndIn(nameAttrStartPattern, schemeContent, searchStartPosition);
        ...
        while (SUBSTRING_NOT_FOUND != nextBlockStart && nextBlockStart < blockEnd && elementNameStart > nextBlockStart) {
            ...
            while (0 != bracesDisbalance) {
                blockEnd = reEndIn(blockEndPattern, schemeContent, lastFoundBlockBoundaryPos + 1);
                nextBlockStart = reIndexIn(blockStartPattern, schemeContent, lastFoundBlockBoundaryPos + 1);
            ...
            elementNameStart = reEndIn(nameAttrStartPattern, schemeContent, blockEnd);
            CHECK_EXT(SUBSTRING_NOT_FOUND != elementNameStart, elementNamesAndIds.clear(), U2_INVALID_SCHEME);

            nextBlockStart = reIndexIn(blockStartPattern, schemeContent, nextBlockStart + 1);
            blockEnd = reEndIn(blockEndPattern, schemeContent, blockEnd + 1);
        }

        elementNameStart = reIndexIn(letterOrNumberPattern, schemeContent, elementNameStart);
        ...
        CHECK_EXT(SUBSTRING_NOT_FOUND != elementNameStart, elementNamesAndIds.clear(), U2_INVALID_SCHEME);
        CHECK_EXT(SUBSTRING_NOT_FOUND != elementNameEnd, elementNamesAndIds.clear(), U2_INVALID_SCHEME);

        const QString elementName = schemeContent.mid(elementNameStart, elementNameEnd - elementNameStart + 1);
        CHECK_EXT(!elementNamesAndIds.contains(elementName), elementNamesAndIds.clear(), U2_INVALID_SCHEME);
        QString elementType;
        ...
        CHECK_EXT(U2_OK == result && typeExists, elementNamesAndIds.clear(), U2_UNKNOWN_ELEMENT);
...
U2ErrorType SchemeWrapper::addActorBindingsBlock(int* position) {
    *position = -1;
    ...
    int metaStartPos = -1;
    QRegularExpressionMatchIterator it = metaStartPattern.globalMatch(schemeContent);
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        int pos = match.capturedStart();
        if (pos > schemeDescriptionEndPos) {
            break;
        }
        metaStartPos = pos;
    }
    ...
    CHECK(SUBSTRING_NOT_FOUND != lastBlockEndPos, U2_INVALID_SCHEME);
    const QString dataflowDescription = Constants::NEW_LINE + Constants::NEW_LINE + HRSchemaSerializer::makeBlock(Constants::ACTOR_BINDINGS, Constants::NO_NAME, QString()) + Constants::NEW_LINE;
...
U2ErrorType SchemeWrapper::setUniqueElementNameAndId(const QString& elementType,
    ...
    if (elementNamesAndIds.contains(elementName)) {
        elementName = WorkflowUtils::createUniqueString(elementName, " ", elementNamesAndIds.keys());
        elementId = WorkflowUtils::createUniqueString(elementId, Constants::DASH, elementNamesAndIds.values());
    }
    return U2_OK;
}
...
const QRegularExpression letterOrQuotePattern(R"([\w"])");
    ...
    int outerBlockStartPos = -1;
    QRegularExpression re(BLOCK_START_PATTERN);
    QRegularExpressionMatchIterator it = re.globalMatch(schemeContent.left(elementTypeAttrStartPos + 1));
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        outerBlockStartPos = match.capturedStart();
    }
...
U2ErrorType SchemeWrapper::setElementAttributeInRange(const QString& attributeName,
    ...
    U2ErrorType result = getAttributeValuePositionFromRange(attributeName, &valueStart, &valueEnd);
    CHECK(U2_OK == result, result);
    if (replaceIfExists && SUBSTRING_NOT_FOUND != valueStart && SUBSTRING_NOT_FOUND != valueEnd) {
        result = replaceStringInScheme(valueStart, valueEnd - valueStart, HRSchemaSerializer::valueString(attributeValue));
    } else {
...
U2ErrorType SchemeWrapper::getBlockBoundaries(const QString& blockName, int* start, int* end)
    ...
    QRegularExpressionMatch match = blockStartPattern.match(schemeContent, *start);
...
U2ErrorType SchemeWrapper::validatePortAndSlot(const QString& elementName,
    ...
    if (!portName.isEmpty()) {
        result = WorkflowElementFacade::doesElementHavePort(elementType, portName, &hasQuality);
        CHECK(U2_OK == result, result);
        CHECK(hasQuality, U2_INVALID_NAME);
    }
    if (!slotName.isEmpty()) {
        if (!portName.isEmpty()) {
            result = WorkflowElementFacade::doesElementHaveInputSlot(elementType, portName, slotName, &hasQuality);
            CHECK(U2_OK == result, result);
            CHECK(hasQuality, U2_INVALID_NAME);
        } else {
            result = WorkflowElementFacade::doesElementHaveOutputSlot(elementType, slotName, &hasQuality);
            CHECK(U2_OK == result, result);
            CHECK(hasQuality, U2_INVALID_NAME);
        }
    }
    ...
    if (SUBSTRING_NOT_FOUND == blockStart && createIfNotExists) {
        ...
        do {
            ...
            result = getAttributeValuePositionFromRange(Constants::DATASET_NAME,
                                                        ...
                                                        &datasetNameEnd);
            CHECK(U2_OK == result, result);
            CHECK(SUBSTRING_NOT_FOUND != datasetNameStart && SUBSTRING_NOT_FOUND != datasetNameEnd, U2_INVALID_STRING);
...
void SchemeWrapper::skipComments() {
    ...
    while (SUBSTRING_NOT_FOUND != commentStartPosition && schemeHeaderStart != commentStartPosition) {
...
U2ErrorType SchemeWrapper::getSchemeDescriptionStart(int* pos) const {
    QRegularExpression blockStartPattern(BLOCK_START_PATTERN);
    QRegularExpression schemeNamePattern(R"("[^"]*")");
    *pos = schemeContent.indexOf(Constants::BODY_START);
    CHECK(SUBSTRING_NOT_FOUND != *pos, U2_INVALID_SCHEME);

    QRegularExpressionMatch blockMatch = blockStartPattern.match(schemeContent, *pos);
    int bodyBlockStartPos = (blockMatch.hasMatch()) ? blockMatch.capturedEnd() : SUBSTRING_NOT_FOUND;
    CHECK(SUBSTRING_NOT_FOUND != bodyBlockStartPos, U2_INVALID_SCHEME);

    bodyBlockStartPos += 0;
    QRegularExpressionMatch schemeMatch = schemeNamePattern.match(schemeContent, *pos);
    int schemeNamePos = (schemeMatch.hasMatch()) ? schemeMatch.capturedEnd() : SUBSTRING_NOT_FOUND;
    if (SUBSTRING_NOT_FOUND != schemeNamePos && bodyBlockStartPos > schemeMatch.capturedStart()) {
        if (bodyBlockStartPos < schemeNamePos) {
            QRegularExpressionMatch nextBlockMatch = blockStartPattern.match(schemeContent, schemeNamePos);
            CHECK(nextBlockMatch.hasMatch(), U2_INVALID_SCHEME);
            bodyBlockStartPos = nextBlockMatch.capturedEnd();
        }
    }
...
U2ErrorType SchemeWrapper::updateCommentPositionsOnShift(int shiftStartPosition, int shiftLength) {
    CHECK(0 <= shiftStartPosition && 0 <= shiftStartPosition + shiftLength && schemeContent.length() > shiftStartPosition, U2_NUM_ARG_OUT_OF_RANGE);
...
U2ErrorType SchemeWrapper::getAttributeValuePositionFromRange(const QString& attributeName,
    ...
    QRegularExpression letterOrNumberPattern("\\w");
    QRegularExpression attributeNamePattern("[\\s\\" + Constants::SEMICOLON + "]" + QRegularExpression::escape(attributeName) + "\\s*\\" + Constants::EQUALS_SIGN);
    QRegularExpressionMatch attrMatch = attributeNamePattern.match(schemeContent, *start);
    const int attributePosition = (attrMatch.hasMatch()) ? attrMatch.capturedStart() : SUBSTRING_NOT_FOUND;
    if (SUBSTRING_NOT_FOUND != attributePosition && attributePosition < *end) {
        int valueStartPosition = attrMatch.capturedEnd();
        QRegularExpressionMatch firstLetterMatch = letterOrNumberPattern.match(schemeContent, valueStartPosition);
        const int firstValueSymbolPos = (firstLetterMatch.hasMatch()) ? firstLetterMatch.capturedStart() : SUBSTRING_NOT_FOUND;
        const int firstOpenQuotePos = schemeContent.indexOf(Constants::QUOTE, valueStartPosition);
        const bool quotesExist = (SUBSTRING_NOT_FOUND != firstOpenQuotePos && firstOpenQuotePos < firstValueSymbolPos);
        valueStartPosition = quotesExist ? firstOpenQuotePos : firstValueSymbolPos;
        int valueEndPosition = SUBSTRING_NOT_FOUND;
        if (quotesExist) {
            valueEndPosition = schemeContent.indexOf(Constants::QUOTE, firstOpenQuotePos + 1) + 1;
        } else {
            valueEndPosition = schemeContent.indexOf(Constants::SEMICOLON, valueStartPosition);
        }
...
U2ErrorType SchemeWrapper::getUrlInAttributePositionByName(const QStringList& attributesHierarchy,
    ...
    CHECK(Workflow::BaseAttributes::URL_IN_ATTRIBUTE() == attributesHierarchy[0],
          U2_INVALID_CALL);
    deepestAttributeName = (1 < attributesHierarchy.size()) ? attributesHierarchy.last() : Constants::FILE_URL;
    CHECK(Constants::FILE_URL == deepestAttributeName || Constants::DATASET_NAME == deepestAttributeName || Constants::DIRECTORY_URL == deepestAttributeName, U2_INVALID_NAME);
    QString datasetName;
    ...
    if (Constants::DATASET_NAME == deepestAttributeName && createIfNotExists) {
        ...
        result = getBoundariesOfUrlInAttribute(datasetName, createIfNotExists, &urlAttrStart, &urlAttrEnd);
        CHECK(U2_OK == result, result);
