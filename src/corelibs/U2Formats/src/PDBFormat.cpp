...
void PDBFormat::PDBParser::parseBioStruct3D(BioStruct3D& biostruct, U2OpStatus& ti) {
    ...
    while (!ti.isCoR()) {
        ...
        if (currentPDBLine.startsWith("HELIX ") || currentPDBLine.startsWith("SHEET ") || currentPDBLine.startsWith("TURN  ")) {
...
void PDBFormat::PDBParser::parseAtom(BioStruct3D& biostruct, U2OpStatus&, QList<int>& sequenceIds) {
    ...
    */
    ...
    bool isMolBeingParsedFirstTime = currentModelIndex == 0 || !biostruct.moleculeMap.contains(chainIndex);
    if (isMolBeingParsedFirstTime && atomIsInChain) {
        // Process residue
...
int PDBFormat::PDBParser::returnEndOfNameIndexAndUpdateParserState(const QString& specification) {
    ...
    int index = specification.indexOf(end);
    if (index < 0) {
