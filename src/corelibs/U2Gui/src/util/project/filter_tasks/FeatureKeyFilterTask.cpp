...
void FeatureKeyFilterTask::filterDocument(Document* doc) {
    ...
    QByteArrayList objectIdsToSearchIn;
    QList<GObject*> annotationTablesList = doc->findGObjectByType(GObjectTypes::ANNOTATION_TABLE);
    for (const GObject* obj : qAsConst(annotationTablesList)) {
        objectIdsToSearchIn.append(doc->getIdByObject(obj));
    }
    QMap<U2DataId, QStringList> annotationTablesByFk = featureDbi->getAnnotationTablesByFeatureKey(settings.tokensToShow, stateInfo, objectIdsToSearchIn);
