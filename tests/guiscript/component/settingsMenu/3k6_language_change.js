...
//#exitOnEnding=true
print("Start testing...")
print("Test : settings -> User settings, save language value, after close settings")
Test_path = TestHelper.getEnvValue("TEST_PATH");
print(Test_path);
TestHelper.isDebug = true;
main_delay = 100;
//valueToFind="transl_ru";
valueToFind = "transl_en";
...
findValInComboBox = function (curComboBox, valToFind) {
    ...
    if (curComboBox.itemData(curIndex) == valToFind) return;
    predIndex = -1;
    curIndex = -2;
    while (true) {
...
//test of form's elements-----------------------------------------------------------------
        ...
        curElem = UserApplicationSettingsW.langBox.langCombo;
        if (ni == 0) {
            oldOneValue = curElem.itemData(curElem.currentIndex);
            findValInComboBox(curElem, valueToFind);
        } else {
            if (curElem.itemData(curElem.currentIndex) != valueToFind) {
                print("wrong value in langCombo ComboBox");
                throw "wrong value in langCombo ComboBox";
            }
